param(
    [Parameter(Mandatory=$true, HelpMessage="The Policy subscription")]
    [string]$subscriptionId,

    [Parameter(Mandatory=$true, HelpMessage="The Policy resource group")]
    [string]$resourceGroup,

    [Parameter(Mandatory=$true, HelpMessage="The Policy name")]
    [string]$enterprisePolicyName,

    [Parameter(Mandatory=$true, HelpMessage="The Policy location")]
    [string]$enterprisePolicylocation,

    [Parameter(Mandatory=$true, HelpMessage="Primary virtual network Id")]
    [string]$primaryVnetId,

    [Parameter(Mandatory=$true, HelpMessage="Primary subnet name")]
    [string]$primarySubnetName,

    [Parameter(Mandatory=$true, HelpMessage="Secondary virtual network Id (put N/A if not used, but vnet is required if geo supports 2+ regions)")]
    [string]$secondaryVnetId,

    [Parameter(Mandatory=$true, HelpMessage="Secondary subnet name (put N/A if not used, but subnet is required if geo supports 2+ regions)")]
    [string]$secondarySubnetName
)

# Load the environment script
. ".\scripts\common\EnterprisePolicyOperations.ps1"
. ".\scripts\common\ValidateVnetLocationForEnterprisePolicy.ps1"

function CreateSubnetInjectionEnterprisePolicy {
    #$resourceGroup='NEUTest'
